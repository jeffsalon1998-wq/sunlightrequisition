...
import { INITIAL_REQUISITIONS, INITIAL_INVENTORY } from "../constants";
...
export const getDatabaseStatus = () => {
  return {
    isUsingFallback,
    connectionTested,
    isCloudReady: !!DB_URL && !isUsingFallback
  };
};
...
export const initDatabase = async () => {
  ...
  try {
    ...
    const itemCount = await dbClient.execute("SELECT COUNT(*) as count FROM items");
    if (Number(itemCount.rows[0].count) === 0) {
      for (const item of INITIAL_INVENTORY) {
        await dbClient.execute({
          sql: "INSERT INTO items (id, name, category, uom, unitCost, parStock) VALUES (?, ?, ?, ?, ?, ?)",
          args: [item.id, item.name, item.category, item.unit, item.pricePerUnit, item.minStock]
        });
        
        // Add initial batch for the item
        await dbClient.execute({
          sql: "INSERT INTO batches (item_id, quantity) VALUES (?, ?)",
          args: [item.id, item.stock]
        });
      }
    }
  } catch (err) {
...
export const deleteRequisitionDb = async (id: string) => {
  if (isUsingFallback || !dbClient) {
    const requisitions = await getRequisitions();
  ...
  try {
    await dbClient.execute({
      sql: "DELETE FROM requisitions WHERE id = ?",
      args: [id]
    });
  } catch (err) {
...
export const verifyDepartmentPassword = async (dept: Department, password: string): Promise<boolean> => {
  ...
  try {
    ...
    let key = dept === 'Purchasing' ? 'dept_pass_Admin' : `dept_pass_${dept}`;
    
    // Handle specific database naming mismatches
    if (dept === 'Sports & Recreations') {
      key = 'dept_pass_Sports & Recreation';
    }
