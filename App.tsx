...
function AppContent() {
  ...
  const handleDeleteRequisition = async (id: string) => {
    ...
    try {
      await deleteRequisitionDb(id);
      toast.success('Requisition deleted successfully');
    } catch (err) {
      toast.error('Deletion failed');
      console.error("Failed to delete requisition in DB:", err);
    }
  };
  ...
  if (!defaultDept) {
    return <LoginGate availableDepartments={availableDepartments} onLogin={setDefaultDept} />;
  }
