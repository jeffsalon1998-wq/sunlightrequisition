...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white/80 dark:bg-stone-900 h-16 flex items-center justify-between px-8 border-b border-stone-200/50 dark:border-stone-800 transition-colors duration-300">
      <div className="flex items-center gap-3">
        {dbStatus.connectionTested && (
          <div className="flex items-center gap-2 px-3 py-1 bg-stone-50 dark:bg-stone-950 rounded-full border border-stone-200 dark:border-stone-800">
            <div className={`w-1.5 h-1.5 rounded-full ${dbStatus.isCloudReady ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
            <span className="text-[10px] font-black uppercase tracking-widest text-stone-600 dark:text-stone-400">
              {dbStatus.isCloudReady ? 'Cloud Active' : 'Local Fallback'}
            </span>
          </div>
        )}
      </div>
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 border-l border-stone-200 dark:border-stone-800 pl-6">
          ...
          <div className="w-10 h-10 rounded-full bg-maroon-bg dark:bg-maroon-900 text-white dark:text-gold-text flex items-center justify-center font-bold text-base shadow-md border border-transparent dark:border-maroon-800">
