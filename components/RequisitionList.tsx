...
export default function RequisitionList({ 
  ...
  const [deptFilter, setDeptFilter] = useState<Department | 'All'>(
    defaultDept === 'Purchasing' ? 'All' : (defaultDept || 'All')
  );
  
  useEffect(() => {
    setDeptFilter(defaultDept === 'Purchasing' ? 'All' : (defaultDept || 'All'));
  }, [defaultDept]);
  ...
  const executeDelete = async () => {
    if (!confirmDelete || !onDeleteRequisition) return;
    setIsProcessing(true);
    try {
      await onDeleteRequisition(confirmDelete);
  ...
  const filteredRequisitions = useMemo(() => {
    return requisitions.filter(req => {
      ...
      // If not admin, force filter by department
      const deptVisibility = isAdmin ? (deptFilter === 'All' || req.department === deptFilter) : (req.department === defaultDept);
      
      return matchesSearch && deptVisibility;
    });
  }, [requisitions, searchQuery, deptFilter, isAdmin, defaultDept]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 px-1">
        <div>
          <h2 className="text-2xl font-bold text-stone-900 dark:text-stone-100 tracking-tight">Purchase Request Tracking</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="relative">
          <select 
            ...
            value={isAdmin ? deptFilter : (defaultDept || 'All')}
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                        ...
                        <span className="text-[10px] text-stone-500 dark:text-stone-400 flex items-center gap-1.5 font-medium"><Calendar size={10}/> {new Date(req.date).toLocaleString()}</span>
                        {req.eventDate && (
                    ...
                    <div className="flex items-center gap-3">
                      {isAdmin && (
                        <button 
                          ...
                          className="p-2 text-stone-400 dark:text-stone-500 hover:text-red-600 dark:hover:text-red-500 transition-colors flex items-center justify-center bg-stone-50 dark:bg-stone-800/50 rounded-lg border border-stone-100 dark:border-stone-800"
                      ...
                      {isAdmin && req.status === 'Pending' && (
                        <button 
                          ...
                          className="p-2 text-stone-400 dark:text-stone-500 hover:text-red-500 dark:hover:text-red-400 transition-colors flex items-center justify-center bg-stone-50 dark:bg-stone-800/50 rounded-lg border border-stone-100 dark:border-stone-800"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl border border-stone-100 animate-in zoom-in-95 duration-300">
            <div className="p-8 text-center">
              ...
              <h3 className="text-xl font-black text-stone-900 uppercase tracking-tight mb-2">Delete Requisition?</h3>
              <p className="text-stone-500 text-xs font-medium leading-relaxed">
                Are you sure you want to delete requisition <span className="font-black text-stone-800">{confirmDelete}</span>? This action cannot be undone.
              </p>
            </div>
            <div className="flex border-t border-stone-100">
              <button 
