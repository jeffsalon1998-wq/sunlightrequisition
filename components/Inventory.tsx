  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div 
        ...
        className="bg-white/70 dark:bg-stone-900/70 backdrop-blur-sm rounded-2xl border-t border-b border-white/20 dark:border-stone-800/50 shadow-sm overflow-hidden transition-colors glass-effect"
      >
            ...
            <AnimatePresence mode="popLayout">
              {filteredInventory.map((item, index) => {
                ...
                <motion.div 
                  ...
                  className="p-3 rounded-xl border transition-all hover:shadow-md bg-white/50 dark:bg-stone-900/50 border-white/20 dark:border-stone-800/50 glass-effect"
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center shadow-sm bg-stone-50 dark:bg-stone-800 text-stone-500 dark:text-stone-400 flex-shrink-0">
                      <Layers size={18} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-xs font-bold text-stone-900 dark:text-stone-100 line-clamp-1">{item.name}</h3>
                      <p className="text-[9px] text-stone-400 dark:text-stone-500 font-mono">ID: {item.id}</p>
                    </div>
                    <span className="text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 flex-shrink-0">
                      {item.category}
                    </span>
                  </div>
                  <div className="mt-2 pt-2 border-t border-stone-100 dark:border-stone-800 flex items-end justify-between">
                    <div>
                      <p className="text-[8px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-0">In Stock</p>
                      <div className="flex items-baseline gap-1">
                        <span className="text-lg font-black leading-none text-stone-900 dark:text-stone-100">{item.stock}</span>
                        <span className="text-[9px] font-bold text-stone-400 dark:text-stone-500 uppercase">{item.unit}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
